...
.LFB0:
	...
	subq	$32, %rsp	#,
# factoriel.c:3: int main() {
	movq	%fs:40, %rax	# MEM[(<address-space-1> long unsigned int *)40B], tmp98
	movq	%rax, -8(%rbp)	# tmp98, D.3189
	xorl	%eax, %eax	# tmp98
# factoriel.c:5: 	long long unsigned int result = 1;
	movq	$1, -16(%rbp)	#, result
.L2:
# factoriel.c:8: 		printf("entrer une nombre pour calculer sont factoriel: ");
	leaq	.LC0(%rip), %rax	#, tmp90
	movq	%rax, %rdi	# tmp90,
	movl	$0, %eax	#,
	call	printf@PLT	#
# factoriel.c:9: 		scanf("%d", &number);
...
# factoriel.c:12: 	while (number > 1) {
	...
	jg	.L4	#,
# factoriel.c:17: 	printf("le result est: %llu\n", result);
